...
// MCP feature flags and budgets (always enabled in app-mcp.js)
// Note: mcpEnabled is declared in the loading script (index.html line 193)
const MCP_CHAT_TIMEOUT_MS = 1500;
const MCP_BUILD_TIMEOUT_MS = 3000;
const MCP_MAX_CALLS_CHAT = 2;
const MCP_MAX_CALLS_BUILD = 2;
...
async function addMessage(role, content, isDiagram = false, messageId = null, skipLabel = false) {
    ...
    if (isDiagram) {
        ...
        // SMART MARKDOWN PARSING: Remove conversational code fences before parsing
        let cleanedContent = content;

        // Pattern 1: Code blocks with common conversational keywords (should be plain text)
        const conversationalPatterns = [
            // Conversational intros
            /```(?:text|markdown)?\s*(here['']s|this is|the following|below|above|i['']ve created|i have created|the workflow|the diagram|this design|the proposed)[\s\S]*?```/gi,
            // Descriptive phrases
            /```\s*(the mermaid diagram shows?|this workflow shows?|the design shows?|the proposed workflow)[\s\S]*?```/gi,
            // Explanatory phrases
            /```\s*(this diagram illustrates|the diagram below|the diagram above)[\s\S]*?```/gi,
            // Statement patterns
            /```\s*(i['']ve designed|i have designed|here is the design|here is the workflow)[\s\S]*?```/gi
        ];

        for (const pattern of conversationalPatterns) {
            cleanedContent = cleanedContent.replace(pattern, (match) => {
                // Remove fence markers, keep content
                return match.replace(/^```\w*\n?/gm, '').replace(/```$/gm, '');
            });
        }

        // Pattern 2: Code blocks that are just single-line text in fences
        cleanedContent = cleanedContent.replace(/```(?:json|text)?\s*([a-zA-Z][^\n{]*)```/g, '$1');

        // Pattern 3: Empty code blocks or blocks with only whitespace
        cleanedContent = cleanedContent.replace(/```\s*[\r\n]+\s*```/g, '');

        // Now parse the cleaned markdown
...
Return ONLY the fixed Mermaid code starting with "graph TD" - no markdown fences, no explanation.`;
    ...
    try {
        ...
        const requestBody = {
            ...
            stage: 'diagram_fix' // No tools needed for syntax fixes
        };
...
/**
 * Load comprehensive n8n workflow design principles (30 best practices)
 * Used at diagram stage for informed architecture decisions
 */
function getBuildingWorkflowTips() {
    return `
## n8n Workflow Design Principles (30 Best Practices)

### Core Architecture (Principles 1-2)
1. **Clarity & Maintainability**
   - Name nodes descriptively (e.g., "Remove non-relevant articles" not "Filter")
   - Use centralized configuration as second node
   - Group related fields with dot notation (content.title, content.url)
   - Simplify data structures early with Set nodes

2. **Control Patterns**
   - Use Switch nodes instead of IF nodes (more flexible, named outputs)
   - Don't default to AI agents - sequential workflows often better
   - Understand item linking for proper data flow

### Cost & Performance (Principles 3-4)
3. **Smart AI Model Selection**
   - Start with cheaper models (Gemini Flash, GPT-4o-mini) before expensive ones
   - Use pre-filtering: cheap models eliminate 70-90% before expensive processing
   - Leverage OpenRouter for easy model switching
   - Set monthly spending limits

4. **Testing Optimization**
   - Pin data to avoid repeated API calls during testing
   - Edit pinned data for edge case testing
   - Limit items during testing (3 instead of 50)
   - Use page size=1 trick to correlate inputs/outputs

### Quality Control (Principles 5-6)
5. **Human Oversight**
   - Implement human-in-the-loop for critical decisions
   - Use Telegram for notifications, approvals, workflow management
   - Create backup automations early

6. **Data Enhancement**
   - Implement RAG for context AI wasn't trained on
   - Use structured storage (Airtable, Baserow) over Google Sheets
   - Create reusable data structures

### Error Handling (Principles 11-13)
11. **Data Flow**
    - Create single/multiple items programmatically
    - Use HTTP node with predefined credential types
    - Implement smart filtering after retrieval

12. **Multi-Layer Error Strategies**
    - Use Error Trigger nodes for automated alerts
    - Implement retry logic: 3-5 retries, 5-second delays
    - Use exponential backoff (5s, 10s, 20s, 40s) for critical services
    - Create dedicated error workflows
    - Log all failed attempts

13. **Fallback Mechanisms**
    - Implement alternative paths when services fail
    - Use multiple AI providers as backups
    - Store critical data locally before external operations
    - Design graceful degradation for non-critical failures

### Performance & Scalability (Principles 14-15)
14. **Optimize for Scale**
    - Structure workflows for parallel execution
    - Minimize API calls through efficient configuration
    - Implement robust retry without blocking
    - Migrate from SQLite at 5K-10K daily executions
    - Batch similar operations

15. **Queue Architecture**
    - Multiple n8n instances: main for triggers, workers for execution
    - Transition to PostgreSQL before hitting limits
    - Add Redis for queue mode at 50+ concurrent workflows
    - Set worker concurrency to 5+ to avoid connection exhaustion

### Security & Compliance (Principles 18-19)
18. **Security Best Practices**
    - Use environment variables - never hardcode credentials
    - Encrypt sensitive data in transit and at rest
    - Implement RBAC (role-based access control)
    - Audit workflow executions and access logs
    - Regularly update n8n for security patches

19. **Compliance Design**
    - Build audit trails for regulated industries
    - Implement data retention policies
    - Ensure GDPR/privacy compliance
    - Create automated compliance reporting workflows

### Monitoring (Principles 20-21)
20. **Comprehensive Monitoring**
    - Track execution logs, alert on suspicious behavior
    - Monitor execution time for performance degradation
    - Track resource usage (CPU, memory, DB connections)
    - Monitor business metrics alongside technical metrics

21. **Proactive Alerting**
...
/**
 * Abbreviated principles for early conversation stages (token-efficient)
 */
function getAbbreviatedPrinciples() {
    return `
Key n8n design principles:
- Use descriptive node names that explain their function
- Prefer Switch nodes over IF nodes for conditional logic
- Start with cheaper AI models (Gemini Flash, GPT-4o-mini) before expensive ones
- Implement retry logic: 3-5 retries with 5-second delays
- Use centralized configuration nodes early in workflow
- Group related fields with dot notation (e.g., content.title, content.url)
- Build in human oversight for critical decisions (Telegram notifications)
...
/**
 * Show MCP tool checking message (displayed during API call)
 * Uses same format as Build phase for consistency
 */
function showMcpToolCheckingMessage() {
    if (!mcpEnabled) return null;

    // Create message element directly (similar to thinking message)
    const messageDiv = document.createElement('div');
    messageDiv.className = 'message assistant mcp-checking-message';
    messageDiv.id = 'mcpCheckingMessage';
    ...
    const contentDiv = document.createElement('div');
    contentDiv.className = 'message-content';
    contentDiv.innerHTML = `<strong style="color: #fbbf24;">🔍 MCP Mode Active</strong><br>Checking n8n node capabilities and Context7 documentation...`;
...
/**
 * Display tool call results after response received
 * Uses same addMessage() format as Build phase
 */
function displayToolCallResults(toolCalls) {
    if (!toolCalls || toolCalls.length === 0) return;

    // Categorize tool calls
    const n8nCalls = toolCalls.filter(tc =>
        tc.name.includes('search_nodes') || tc.name.includes('get_node')
    );
    const context7Calls = toolCalls.filter(tc =>
        tc.name.includes('resolve-library') || tc.name.includes('query-docs') || tc.name.includes('Context7')
    );

    // Show n8n-mcp results
    if (n8nCalls.length > 0) {
        const successCount = n8nCalls.filter(tc => tc.success).length;
        addMessage('assistant', `✅ Retrieved ${successCount} result(s) from n8n-mcp`, false, null, true);
    }

    // Show Context7 results
    if (context7Calls.length > 0) {
        const successCount = context7Calls.filter(tc => tc.success).length;
        addMessage('assistant', `✅ Retrieved ${successCount} result(s) from Context7`, false, null, true);
    }
}
...
async function processUserInput(input) {
    ...
    try {
        ...
        // Show MCP checking message if enabled and in relevant stage
        let mcpCheckingMsg = null;
        if (mcpEnabled && (currentStage === 'initial' || currentStage === 'clarifying' || currentStage === 'design_proposed')) {
            mcpCheckingMsg = showMcpToolCheckingMessage();
        }
        ...
        const requestBody = {
            ...
            stage: currentStage // Pass stage for tool_choice decision
        };
        ...
        // Remove checking message and show results
        if (mcpCheckingMsg && mcpCheckingMsg.parentNode) {
            mcpCheckingMsg.remove();
        }

        // Display tool call results if any
        if (data._toolCallsMade && data._toolCallsMade.length > 0) {
            displayToolCallResults(data._toolCallsMade);
        }
        ...
        // OpenAI now handles tool calling natively in the proxy server
        // No need for manual tool_action parsing - just use the response directly
        ...
        // First, extract mermaid diagrams BEFORE cleanup to preserve them
        const mermaidMatch = finalResponse.match(/```\s*mermaid\s*[\r\n]+([\s\S]*?)```/i);
        let mermaidDiagramCode = null;

        if (mermaidMatch) {
            mermaidDiagramCode = mermaidMatch[1];
            // Temporarily replace mermaid block with placeholder
            finalResponse = finalResponse.replace(/```\s*mermaid\s*[\r\n]+([\s\S]*?)```/i, '___MERMAID_DIAGRAM_PLACEHOLDER___');
        }

        // Clean up response artifacts
        // Note: Session 15 backend now formats tool results as plain text, so minimal cleanup needed
        let cleanResponse = finalResponse
            // Remove ONLY truly empty code blocks
            .replace(/```[\s\n\r]*```/g, '')
            // Clean up stray JSON punctuation (minimal)
            .replace(/^\s*[{}\[\]]\s*$/gm, '')
            .trim();

        // Only apply heavy cleanup if we actually see problematic JSON code blocks
        // This handles edge cases where backend fix might not work
        const hasProblematicJson = /```\s*json\s*[\s\S]*?"(?:nodes|connections|name)":\s*[\s\S]*?```/.test(finalResponse);

        if (hasProblematicJson) {
            // This looks like old-style JSON tool results - apply aggressive cleanup
            cleanResponse = finalResponse
                .replace(/```(?:json)?\s*\{[\s\S]*?"(?:nodes|connections|name)":\s*[\s\S]*?```/g, '')
                .replace(/```[\s\n\r]*```/g, '')
                .trim();
        }

        // If cleanup resulted in empty/garbage response, try minimal extraction
        const isGarbageResponse = !cleanResponse ||
            cleanResponse.length < 10 ||  // Too short to be meaningful
            /^[\s{}\[\],;:"`']*$/.test(cleanResponse);  // Only punctuation/whitespace

        if (isGarbageResponse) {
            console.warn('⚠️ Cleanup resulted in empty response, using original response');
            // Try to extract just the text content from original
            cleanResponse = finalResponse
                .replace(/```[\s\S]*?```/g, '')  // Remove all code blocks
                .trim();

            // If still empty, indicate an error
        ...
        // Additional cleanup: remove any stray unclosed code fences
        assistantResponse = assistantResponse
            .replace(/```\s*$/g, '')  // Trailing unclosed fence
            .replace(/^\s*```/g, '')  // Leading unclosed fence
            .trim();

        // Restore mermaid diagram if it was extracted
        if (mermaidDiagramCode) {
            assistantResponse = assistantResponse.replace('___MERMAID_DIAGRAM_PLACEHOLDER___', '```mermaid\n' + mermaidDiagramCode + '\n```');
        }

        // Remove conversational code blocks masquerading as code (dark blue boxes)
        assistantResponse = assistantResponse
            // Pattern 1: Code blocks with conversational intros (most common)
            .replace(/```(?:json|text)?\s*(here['']s|this is|the following|below|above is|i['']ve created|i have created|the workflow|the diagram|this design)[\s\S]*?```/gi, (match) => {
                // Remove fence markers, keep the text content
                return match.replace(/^```\w*\n?/gm, '').replace(/```$/gm, '');
            })
            // Pattern 2: Single-line conversational code blocks (text without JSON structure)
            .replace(/```(?:json|text)?\s*([a-z][^{}\n]{10,})```/gi, '$1')
            // Pattern 3: Empty or whitespace-only blocks
            .replace(/```\s*[\r\n]+\s*```/g, '')
            .trim();
        ...
        // Check if response contains a Mermaid diagram (after potential restoration)
        const restoredMermaidMatch = assistantResponse.match(/```\s*mermaid\s*[\r\n]+([\s\S]*?)```/i);

        if (restoredMermaidMatch) {
            // Extract the diagram and the rest of the message
            const diagramCode = restoredMermaidMatch[1];
            let textBefore = assistantResponse.substring(0, restoredMermaidMatch.index);
            let textAfter = assistantResponse.substring(restoredMermaidMatch.index + restoredMermaidMatch[0].length);

            // Clean any remaining code blocks from text portions
            textBefore = textBefore
                .replace(/```[\s\S]*?```/g, '')
                .replace(/```[\s\n\r]*$/g, '')  // Trailing unclosed fence
                .trim();
            textAfter = textAfter
                .replace(/```[\s\S]*?```/g, '')
                .replace(/^[\s\n\r]*```/g, '')  // Leading unclosed fence
                .trim();
...
async function handleDiagramItClick(retryCount = 0) {
    ...
    try {
        ...
        if (retryCount < maxRetries) {
            ...
            addMessage('assistant', `❌ Failed to generate diagram after ${maxRetries} attempts. Error: ${error.message}`);
            lastFailedPhase = 'diagram';
            addRetryButton('diagram');
        }
    }
}
...
Example tone: "I designed this workflow to start with X because... I chose to use a Switch node here instead of IF nodes because... I placed the error handling at this point because..."`;
    ...
    try {
        ...
        const requestBody = {
            ...
            stage: 'explanation' // Tools optional for explanations
        };
        ...
        if (response.ok) {
            ...
            if (explanation.trim()) {
                // Remove any leaked code blocks or mermaid syntax
...
async function handleBuildItClick() {
    ...
    // VALIDATION: Ensure diagram was generated first
    if (currentStage !== 'diagram_generated' && currentStage !== 'json_generated') {
        addMessage('assistant', '⚠️ **Please create a diagram first**\n\nYou need to click "Diagram it!" before building the workflow JSON. This ensures we have a visual design to work from.');
        return;
    }
...
// Extract last diagram from conversation history
function extractLastDiagram() {
...
// Extract node types from Mermaid diagram
function extractNodeTypesFromDiagram(diagram) {
    if (!diagram) return [];

    const nodeTypes = new Set();

    // Map common names to n8n node types
    const nodeTypeMap = {
    ...
    // Check each pattern
    for (const [key, nodeType] of Object.entries(nodeTypeMap)) {
...
// Fetch node schemas from n8n-mcp
async function fetchNodeSchemas(nodeTypes) {
    if (!nodeTypes || nodeTypes.length === 0) return {};

    console.log('🔍 Fetching schemas for node types:', nodeTypes);
        ...
        try {
            ...
            if (result.content && Array.isArray(result.content) && result.content.length > 0) {
                ...
                if (textContent && textContent.text) {
                    try {
                        schemas[nodeType] = JSON.parse(textContent.text);
                        console.log(`✅ Fetched schema for ${nodeType}`);
                    } catch (parseErr) {
                        console.warn(`⚠️ Failed to parse schema for ${nodeType}:`, parseErr.message);
                    }
                } else {
                    console.warn(`⚠️ No text content in schema response for ${nodeType}`);
                }
            } else if (result.error) {
                console.warn(`⚠️ Failed to fetch schema for ${nodeType}:`, result.error);
            } else {
                console.warn(`⚠️ Unexpected response format for ${nodeType}:`, JSON.stringify(result).slice(0, 100));
            }
        } catch (err) {
            console.error(`❌ Error fetching schema for ${nodeType}:`, err);

            // Detect connection errors specifically
            if (err.message && err.message.includes('Failed to fetch')) {
                // Show user-friendly connection error (only once for all schemas)
                if (!window.mcpConnectionErrorShown) {
                    window.mcpConnectionErrorShown = true;
                    addMessage('assistant', `⚠️ **Cannot Connect to Proxy Server**\n\nThe MCP schema fetching requires the proxy server to be running.\n\nPlease ensure the server is started:\n\`\`\`\ncd workflow-planner && node proxy-server.js\n\`\`\`\n\nThen refresh this page and try again.`, false, null, false);
                }
            }
        }
    }
...
async function generateWorkflowJSON(workflowDesign, retryCount = 0) {
    ...
    // Fetch node schemas from n8n-mcp if MCP is enabled (only on first attempt)
    let schemaContext = '';
    if (mcpEnabled && retryCount === 0) {
        try {
            const diagram = extractLastDiagram();
            if (diagram) {
                const nodeTypes = extractNodeTypesFromDiagram(diagram);
                if (nodeTypes.length > 0) {
                    console.log('📦 MCP Mode: Fetching schemas for', nodeTypes.length, 'node types');
                    addMessage('assistant', `🔍 **MCP Mode Active**\n\nFetching exact schemas for ${nodeTypes.length} node types from n8n-mcp...`);

                    const schemas = await fetchNodeSchemas(nodeTypes);
                    const schemaCount = Object.keys(schemas).length;

                    if (schemaCount > 0) {
                        console.log(`✅ Retrieved ${schemaCount} node schemas from n8n-mcp`);
                        addMessage('assistant', `✅ Retrieved ${schemaCount} validated schemas from n8n-mcp`, false, null, true);

                        // Format schemas for injection into prompt
                        schemaContext = '\n\nEXACT NODE SCHEMAS (from n8n-mcp):\n';
                        for (const [nodeType, schema] of Object.entries(schemas)) {
                            schemaContext += `\n${nodeType}:\n${JSON.stringify(schema, null, 2)}\n`;
                        }
                        schemaContext += '\nIMPORTANT: Use ONLY the exact parameter names and types from these schemas.\n';
                    }
                }
            }
        } catch (err) {
            console.error('Error fetching n8n-mcp schemas:', err);
            addMessage('assistant', '⚠️ Could not fetch schemas from n8n-mcp, proceeding with built-in knowledge', false, null, true);
        }
    }
...
Return ONLY the JSON, no other text or formatting.`;
    ...
    try {
        ...
        if (validationResult.valid) {
            // Additional validation with n8n-mcp if enabled
            if (mcpEnabled && retryCount === 0) {
                try {
                    console.log('🔍 Validating workflow with n8n-mcp...');
                    const mcpValidationResp = await fetch('/api/mcp/call', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            tool: 'n8n-mcp.validate_workflow',
                            arguments: {
                                workflow: validationResult.json
                            },
                            phase: 'build'
                        })
                    });

                    const mcpValidation = await mcpValidationResp.json();

                    if (mcpValidation.status === 'ok') {
                        const result = JSON.parse(mcpValidation.content);
                        if (result.valid) {
                            console.log('✅ n8n-mcp validation passed');
                            addMessage('assistant', '✅ Workflow validated successfully with n8n-mcp', false, null, true);
                        } else {
                            console.warn('⚠️  n8n-mcp validation warnings:', result.warnings);
                            if (result.warnings && result.warnings.length > 0) {
                                // Format warnings
                                const formattedWarnings = result.warnings.slice(0, 3).map(w => {
                                    const nodeName = w.node ? ` in node "${w.node}"` : '';
                                    return `- **${w.severity}**${nodeName}: ${w.message}`;
                                }).join('\n');

                                addMessage('assistant', `⚠️ **Validation Warnings from n8n-mcp:**\n\n${formattedWarnings}${result.warnings.length > 3 ? `\n\n...and ${result.warnings.length - 3} more warnings` : ''}`, false, null, true);
                            }
                        }
                    }
                } catch (err) {
                    console.error('Error validating with n8n-mcp:', err);
                }
            }
        ...
        if (error.message && error.message.includes('Failed to fetch')) {
            // This is a connection refused error - proxy server not running
            addMessage('assistant', `⚠️ **Cannot Connect to Proxy Server**\n\nThe proxy server must be running for workflow generation to work.\n\n**To fix this:**\n\n1. Open a terminal and run:\n\`\`\`bash\ncd workflow-planner && node proxy-server.js\n\`\`\`\n\n2. Keep that terminal open (the server must stay running)\n\n3. Refresh this page\n\n4. Try clicking "Let's Build It!" again\n\n**Why this happens:** The proxy server handles secure communication with the Claude API. Without it, no workflow generation can occur.`, false, null, false);
            throw new Error('Proxy server connection failed');
        }
...
function downloadWorkflowJSON(jsonWorkflow) {
    try {
        // Store workflow for potential retry
        retryContext.jsonWorkflow = jsonWorkflow;
        ...
        addMessage('assistant', `Sorry, I encountered an error downloading the file: ${error.message}. Please try again.`);
        lastFailedPhase = 'download';
        addRetryButton('download');
    }
}

// Common proper nouns and service names for workflow automation
...
function setRecordingState(recording, button, input) {
    ...
    if (recording) {
        ...
        // Add recording indicator to the button group (same line as buttons)
...
- Do NOT use code blocks or code examples
- Do NOT include JSON, technical syntax, or raw code in responses
- Keep responses natural and readable`;

    // Use full principles only when designing workflows (token optimization)
    const designPrinciples = (currentStage === 'ready_for_diagram' || currentStage === 'diagram_generated')
        ? getBuildingWorkflowTips()
        : getAbbreviatedPrinciples();

    if (currentStage === 'initial') {
        const mcpTools = mcpEnabled ? `
AVAILABLE MCP TOOLS:
You have access to n8n-mcp tools and Context7 for verifying capabilities and implementation details:
- n8n-mcp.search_nodes: Search for n8n nodes by keyword (e.g., "slack", "webhook", "database")
- n8n-mcp.get_node: Get detailed information about a specific n8n node type
- Context7.resolve-library-id: Resolve library/package names for external frameworks/libraries
- Context7.get-library-docs: Get up-to-date documentation for libraries and frameworks

WHEN TO USE TOOLS:

**Use n8n-mcp tools when:**
- User mentions a specific integration (e.g., "Slack", "Airtable", "PostgreSQL") → Use search_nodes to verify n8n support
- User asks "Can n8n integrate with X?" → Use search_nodes to check
- User describes an integration but you're uncertain → Use search_nodes to confirm

**Use Context7 when:**
- User mentions custom logic, data transformation, or processing → Use Context7 for JavaScript/TypeScript patterns
- User wants to parse data, call APIs, or manipulate JSON → Use Context7 for modern JS libraries (axios, lodash, etc.)
- User mentions specific frameworks (Express, React, etc.) → Use Context7 for implementation details
- User needs validation, formatting, or data enrichment → Use Context7 for utility libraries

**Context7 Tool Examples:**
- To get library docs: Use \`query-docs\` with libraryId (e.g., "/vercel/next.js") and query (the user's question)
- To resolve library name: Use \`resolve-library-id\` with libraryName (e.g., "axios") and query (the user's question)

⚠️ CRITICAL RULE: NO CODE BLOCKS IN RESPONSES ⚠️
- Even when you call tools and get structured results
- You MUST convert that information into plain conversational text
- DO NOT use \`\`\`json or \`\`\`code blocks to display tool results
- DO NOT include raw JSON or structured data from tools
- ALWAYS reference findings in normal conversational language
- Tool results are for YOUR understanding, not for display to the user

**IMPORTANT - HOW TO USE TOOL RESULTS:**
When you call MCP tools and get results:
- ✅ USE the information to ask better questions
- ✅ REFERENCE capabilities conversationally
- ✅ EXPLAIN what's possible in plain text
- ❌ DO NOT display tool results as JSON/code blocks
- ❌ DO NOT include structured data in responses
- ❌ DO NOT show raw tool output

**Example of WRONG response:**
"Here's what I found: \`\`\`json {\\"nodes\\": [...]} \`\`\` What do you need?"

**Example of CORRECT response:**
"n8n has a Slack node that can post messages and a trigger that responds to events. Which would you like to use?"

Remember: Think of tool results as your private research notes. Your response is explaining the research to the user in plain English - don't show the notes, explain what you learned.
` : '';
...
**INTELLIGENCE-GATHERING + CLARIFYING QUESTIONS:**

Your goal: Ask HIGHLY INFORMED clarifying questions by first gathering technical intelligence.

**Step 1: Intelligence Gathering (If Context Provided)**

If the user mentions specific systems, APIs, transformations, or frameworks:
- Use \`n8n-mcp.search_nodes\` to verify n8n support and get specific node details
- Use \`Context7.get-library-docs\` for library patterns and best practices
- Use \`Context7.resolve-library-id\` to find current documentation

ONLY call tools if you have specific technical details to research.

**Step 2: ALWAYS Ask Clarifying Questions (After Intelligence Gathering)**

AFTER gathering any intelligence (or immediately if no tools needed), provide 2-3 clarifying questions that:
- Are informed by the intelligence you gathered (show you know specifics)
- Cover: specific systems/APIs, manual process, volume/frequency, edge cases
- Demonstrate understanding of their technical context

**Example Ideal Response:**
User: "Automate Slack notifications for Shopify orders"
→ You call: n8n-mcp.search_nodes for both "shopify" and "slack"
→ You respond: "I found n8n has dedicated Shopify and Slack nodes. Questions: 1) Use Shopify's 'New Order' trigger or webhook? 2) Simple Slack message or full app integration? 3) Expected hourly volume?"

${designPrinciples}`;
    } else if (currentStage === 'clarifying') {
        const mcpTools = mcpEnabled ? `
AVAILABLE MCP TOOLS:
You have access to n8n-mcp tools and Context7 for verifying capabilities and getting implementation guidance:
- n8n-mcp.search_nodes: Search for n8n nodes by keyword (e.g., "slack", "webhook", "database")
- n8n-mcp.get_node: Get detailed information about a specific n8n node type
- Context7.resolve-library-id: Resolve library/package names for external frameworks/libraries
- Context7.get-library-docs: Get up-to-date documentation for libraries and frameworks
...
**Use n8n-mcp tools when:**
- User mentions a specific integration (e.g., "Slack", "PostgreSQL") → Use search_nodes to verify n8n support
- User asks about integration capabilities → Use search_nodes to check
- Unclear which n8n node handles a specific service → Use search_nodes to find it

**Use Context7 when:**
- User describes data transformation (e.g., "convert XML to JSON", "filter data", "aggregate") → Use Context7 for relevant libraries
- User mentions API calls or HTTP requests → Use Context7 for axios, node-fetch, or request libraries
- User needs to parse/format data (CSV, XML, dates, etc.) → Use Context7 for parsing libraries
- User mentions validation, sanitization, or data cleaning → Use Context7 for validator, joi, etc.
- User wants to implement custom logic in Code nodes → Use Context7 for JavaScript best practices
- User mentions specific frameworks or libraries by name → Use Context7 for their documentation

**Context7 Tool Examples:**
- To get library docs: Use \`query-docs\` with libraryId and query (user's question)
- To resolve library name: Use \`resolve-library-id\` with libraryName and query (user's question)
...
- Continue using tools to gather information
- Reference findings conversationally in plain text
- NEVER display tool results as code/JSON blocks
- Tool results are for YOUR understanding, not for display
` : '';

        return `${basePrompt}

Continue gathering information about the user's workflow needs. This is your final round of clarifying questions before proposing a design.
...
**INTELLIGENCE-GATHERING + FINAL QUESTIONS:**

Your goal: Gather any remaining technical details needed for a robust design proposal.

**Step 1: Intelligence Gathering (If Needed)**

If the user provides new technical details in their response:
- Use \`n8n-mcp.search_nodes\` to verify specific node capabilities
- Use \`n8n-mcp.get_node\` to get detailed configuration parameters
- Use \`Context7.get-library-docs\` for implementation patterns and best practices

**Step 2: Provide Design Proposal**

AFTER gathering any additional intelligence, provide a clear, concise design proposal describing:
1. The overall workflow architecture
2. Key steps and decision points
3. Systems/integrations involved (with specific n8n nodes when applicable)
...
AVAILABLE MCP TOOLS:
You have access to n8n-mcp tools and Context7 for verifying capabilities and getting implementation guidance:
- n8n-mcp.search_nodes: Search for n8n nodes by keyword
- n8n-mcp.get_node: Get detailed information about a specific n8n node type
- Context7.resolve-library-id: Resolve library/package names
- Context7.get-library-docs: Get up-to-date documentation

WHEN TO USE TOOLS:
- User mentions specific integrations → Use search_nodes to verify
- User describes data transformations → Use Context7 for libraries
- User asks about custom logic → Use Context7 for best practices

**Context7 Tool Examples:**
- Use \`query-docs\` with libraryId and query (user's question) for documentation
- Use \`resolve-library-id\` with libraryName and query (user's question) to find libraries
...
**TOOL RESULT HANDLING:**
- Use tools to verify node capabilities and implementation details
- Describe capabilities and options in plain text
- NEVER include raw tool output in responses
- Tool results are for YOUR understanding, not for display
` : '';
...
${designPrinciples}`;
        ...
        const mcpTools = mcpEnabled ? `
AVAILABLE MCP TOOLS:
You have access to n8n-mcp tools and Context7 for final verification and implementation guidance:
- n8n-mcp.search_nodes: Find n8n nodes for specific integrations
- n8n-mcp.search_templates: Find similar workflow examples for reference
- Context7.resolve-library-id: Resolve library/package names for external frameworks/libraries
- Context7.get-library-docs: Get documentation for libraries and implementation patterns

WHEN TO USE TOOLS:

**Use n8n-mcp tools when:**
- Uncertain which n8n node to use for a specific integration → Use search_nodes to find it
- Want to find similar workflow examples → Use search_templates for inspiration

**Use Context7 when:**
- Workflow requires data transformation in Code nodes → Use Context7 for transformation libraries (lodash, moment, etc.)
- Workflow needs custom API calls → Use Context7 for HTTP client libraries (axios, node-fetch)
- Workflow involves data parsing (JSON, CSV, XML) → Use Context7 for parsing libraries
- Workflow requires validation or formatting → Use Context7 for utility libraries
- Custom business logic needed in Code nodes → Use Context7 for JavaScript patterns and best practices
- User mentioned specific libraries or frameworks → Use Context7 for their documentation

**Context7 Tool Examples:**
- Use \`query-docs\` with libraryId and query to get library documentation
- Use \`resolve-library-id\` with libraryName and query to find Context7-compatible library IDs

⚠️ CRITICAL RULE: NO CODE BLOCKS IN RESPONSES ⚠️
- Even when you call tools and get structured results
- You MUST convert that information into plain conversational text
- DO NOT use \`\`\`json or \`\`\`code blocks to display tool results (EXCEPT for the mermaid diagram itself)
- DO NOT include raw JSON or structured data from tools in text explanations
- ALWAYS reference findings in normal conversational language

**TOOL RESULT HANDLING:**
- Use tools to check node compatibility and verify implementation details
- Explain what's possible in plain text
- NO code blocks, NO JSON displays in text (only mermaid diagram code block is allowed)
- Tool results are for YOUR understanding, not for display
` : '';
...
**IMPORTANT:** Respond directly with the mermaid diagram in a code block. DO NOT use tool_action blocks - just provide the diagram directly, optionally followed by a brief explanation.
...
Optional brief explanation text can follow the diagram.
...
function saveStateForToggle() {
    try {
        const state = {
            ...
            clarificationCount,
            diagramCount,
            // currentSessionId removed - only used in database tracking, not needed for state preservation
...
function restoreStateFromToggle() {
    ...
    try {
        ...
        // currentSessionId removed - not needed for state preservation
...
function initializeApp() {
    console.log('🚀 Initializing app-mcp.js...');
...
// Show warning banner when proxy server is not running
function showProxyServerWarning() {
    const warningDiv = document.createElement('div');
    warningDiv.id = 'proxyServerWarning';
    ...
    warningDiv.innerHTML = `
        <div style="max-width: 800px; margin: 0 auto;">
            ...
            <div style="font-size: 14px; margin-bottom: 12px;">
                The backend server is required for this app to work. Please start it with:
            </div>
            <div style="background: rgba(0,0,0,0.2); padding: 8px 12px; border-radius: 4px; font-family: 'Monaco', 'Courier New', monospace; font-size: 13px; display: inline-block; margin-bottom: 8px;">
                cd workflow-planner && node proxy-server.js
            </div>
